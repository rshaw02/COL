	...
	public sealed class CardGame : IDisposable
	{
		...
		private void NextJudge()
		{
			lock (_playerListLock)
			{
				...
				var judge = Judge;
				int n = PlayerCount;
				var troubleMakers = _players.Select((p, i) => (index: i, player: p)).Where(t => t.player.IsTroubleMaker).ToArray();
				...
				else
				{
					...
					if (troubleMakers.Length > 0 && _rng.Next(0, 2) == 0)
					{
						int offset = _rng.Next(troubleMakers.Length);
						for (int i = 0; i < troubleMakers.Length; i++)
						{
							int index = (offset + i) % troubleMakers.Length;
							// Ignore AFK troubleMakers
							if (troubleMakers[index].player.IsAfk) continue;
							_judgeIndex = troubleMakers[index].index;
							return;
						}
					}
		...
		public string CreatePlayerName(string requestedName, Player player)
		{
			...
			player.IsTroubleMaker |= troubleMaker;
		...
		internal void UpdateSkipVotes()
		{
			lock (_skipCheckLock)
			{
				...
				lock (_playerListLock)
				{
					eligibleSkipVoterCount = _players.Count(p => !p.IsAutonomous && !p.IsAfk && !p.IsTroubleMaker);
		...
		private void AssignTrophies()
		{
			foreach (var player in _players)
			{
				if (player.IsTroubleMaker) continue;
